...
const Collaborator = ({roomId,creatorId,collaborator,email,user}:CollaboratorProps) => {
   ...
   const removedocumentHandler=async(type:string)=>{
    setLoading(true);
   ...
   return (
    <li className='flex items-center justify-between gap-2 py-3'>
        <div className='flex gap-2'>
            <Image src={collaborator.avator} alt={collaborator.name} width={36} height={36} className='size-9 rounded-full'/>
            <div>
        ...
        {creatorId===collaborator.id?(
            ...
            <div className='flex items-center'>
                ...
                <Button className='text-sm text-blue-100' onClick={()=>removedocumentHandler(collaborator.email)}>Remove</Button>
            </div>
        )}
    </li>
  )
